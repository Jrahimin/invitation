    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li role="presentation" class="active"><a href="#invitations" aria-controls="invitations" role="tab"
                                                          data-toggle="tab">Invitations</a></li>
            </ul>
                        ...
                        @if (count($invitations) > 0)
                            @foreach ($invitations as $invitation)
                                <tr data-entry-id="{{ $invitation->id }}">
                                    <td field-key='event'>{{ $invitation->mobile_number ?? 'n/a' }}</td>
                                    <td field-key='email'>{{ $invitation->email ?? 'n/a' }}</td>
                                    <td field-key='relation'>{{ $invitation->relation ?? 'n/a' }}</td>
                                    <td field-key='people_count'>{{ $invitation->people_count ?? 'n/a' }}</td>
                                    <td field-key='sent_at'>{{ $invitation->sent_at ?? 'n/a' }}</td>
                                    <td field-key='accepted_at'>{{ $invitation->accepted_at ?? 'n/a' }}</td>
                                    <td field-key='rejected_at'>{{ $invitation->rejected_at ?? 'n/a' }}</td>
                                    @if( request('show_deleted') == 1 )
                                        <td>
                                            {!! Form::open(array(
